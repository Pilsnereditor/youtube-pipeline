...
function populateVideoDropdowns() {
  ...
  if (currentVal && filteredVideos.some(vid => String(vid.id) === currentVal)) {
    schedVideoSelect.value = currentVal;
  }
}
...
function populateThumbnailDropdowns() {
  ...
  if (currentVal && filteredThumbs.some(thumb => String(thumb.id) === currentVal)) {
    schedThumbSelect.value = currentVal;
  }
}
...
async function saveToMediaLibrary() {
  ...
  const videoId = parseInt(select.value, 10);
  if (!videoId) {
    showToast('Please select a video from the library first.', 'error');
    return;
  }
  const title = document.getElementById('schedTitle').value.trim();
  const description = document.getElementById('schedDesc').value.trim();
  const tags = document.getElementById('schedTags').value.trim();
  try {
    showToast('Saving video details...', 'info');
    const res = await fetch(`${API_BASE}/media/videos/${videoId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ title, description, tags })
    });
    if (!res.ok) throw new Error(await res.text());
    showToast('Video details saved to Media Library successfully!', 'success');
    await loadMediaVideos();
  } catch (err) {
    showToast('Failed to save to Media Library: ' + err.message, 'error');
  }
}
